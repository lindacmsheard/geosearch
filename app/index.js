...
var storeLocationDataUrl = '/app/data/ContosoCoffee.txt';
var assetDataUrl = '/app/data/searchresult.json';

//The URL to the icon image. 
var iconImageUrl = '/app/images/CoffeeIcon.png';

//An array of country region ISO2 values to limit searches to.
var countrySet = ['US', 'CA', 'GB', 'FR','DE','IT','ES','NL','DK'];      

var map, popup, datasource, iconLayer, AssetBboxLayer, AssetBboxLayerFill, centerMarker, searchURL;
var listItemTemplate = '<div class="listItem" onclick="itemSelected(\'{id}\')"><div class="listItem-title">{title}</div>{city}<br />Open until {closes}<br />{distance} miles away</div>';
var assetTemplate = '<div class="listItem" onclick="itemSelected(\'{id}\')"><div class="listItem-title">{title}</div>{city}<br />Latitude: {lat}<br />Longitude: {lon}</div>';
...
function initialize() {
    ...
    map.events.add('ready', function () {
        ...
        AssetBboxLayerFill = new atlas.layer.PolygonLayer(datasource, null, {
            fillColor: 'rgba(0, 200, 200, 0.1)',
            filter: ['==', ['geometry-type'], 'Polygon']
        });

        AssetBboxLayer = new atlas.layer.LineLayer(datasource, null, {
            strokeColor: 'blue',
            strokeWidth: 2,
            filter: ['==', ['geometry-type'], 'Polygon']
        });

        map.layers.add([AssetBboxLayer, AssetBboxLayerFill]);

        // var AssetCentroids = new atlas.layer.SymbolLayer(datasource, null, {
        //     iconOptions: {
        ...
        map.imageSprite.add('myCustomIcon', iconImageUrl).then(function () {
            ...
            iconLayer = new atlas.layer.SymbolLayer(datasource, null, {
                ...
                filter: ['all', ['==', ['geometry-type'], 'Point'], ['!', ['has', 'point_cout']]]
            });
...
function loadAssetData() {
    ...
    fetch(assetDataUrl)
        ...
        .then(function (results) {
            ...
            var features = [];
            console.log(results);
            console.log(results.value[0].Location.Latitude)
            console.log(results.value[0].Location.Longitude)
            var i;

            for (i = 0; i < results.value.length; i++) {
                
                features.push(new atlas.data.Feature(new atlas.data.Point([results.value[i].Location.Longitude, results.value[i].Location.Latitude]), {
                    AssetId: results.value[i].AssetId,
                    AssetName: results.value[i].AssetName,
                    Category: results.value[i].Category
                }));

                console.log("adding point " + i + " to features: Latitude: " + parseFloat(results.value[i].Location.Latitude) + " Longitude: " + parseFloat(results.value[i].Location.Longitude))

                features.push(new atlas.data.Feature(getBboxPolygon(results.value[i].Location.bbox), {
                    AssetId: results.value[i].AssetId,
                    AssetName: results.value[i].AssetName,
                    Category: results.value[i].Category
                }));
...
function getBboxPolygon(bbox) {
    var bboxPolygon = new atlas.data.Polygon([[
        [bbox[0], bbox[1]],
        [bbox[2], bbox[1]],
        [bbox[2], bbox[3]],
        [bbox[0], bbox[3]],
...
function updateListItems() {
    ...
    if (camera.zoom < maxClusterZoomLevel) {
        ...
        data.forEach(function (shape) {
            if (shape instanceof atlas.Shape && shape.getType() === 'Point') {
...
function showPopup(shape) {
    ...
    html.push('<div class="popupTitle">',
        ...
        //Convert the closing time into a nicely formated time.
        //getOpenTillTime(properties),
        'Data Asset type: shapefile',
        //Add the distance information.  
        '<br/>', distance,
        ' miles away',
        '<br /><img src="/app/images/PhoneIcon.png" title="Phone Icon"/><a href="tel:',
        properties['Phone'],
        '">', 
        properties['Phone'],
        '</a>'
    ...
    if (properties['IsWiFiHotSpot'] || properties['IsWheelchairAccessible']) {
        ...
        if (properties['IsWiFiHotSpot']) {
            html.push('<img src="/app/images/WiFiIcon.png" title="Wi-Fi Hotspot"/>');
        }

        if (properties['IsWheelchairAccessible']) {
            html.push('<img src="/app/images/WheelChair-small.png" title="Wheelchair Accessible"/>');
        }
    }
